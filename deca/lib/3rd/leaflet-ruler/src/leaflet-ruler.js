(function(factory, window){
  ...
  L.Control.Ruler = L.Control.extend({
    ...
    _calculateBearingAndDistance: function() {
      ...
      let distance = 0;
      let brng = 0;
      if(this.options.coordinateSystem.type === 'simple')
      {
        let y = (f2 - f1);
        let x = (l2 - l1);
        let R = this.options.lengthUnit.factor ? this.options.lengthUnit.factor : 1.0;

        distance = R * Math.sqrt(x*x + y*y);

        brng = (Math.atan2(y, x) * 180.0 / Math.PI + 90.0) % 360.0;
      }
      else
      {
        // haversine formula
